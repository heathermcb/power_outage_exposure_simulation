...
# in the same way the real data is cleaned, to get the same metrics that
# we have for the real dataset. 
...
clean_simulated_data <- function(p){
  ...
  sim_dat_b <-
    sim_dat_b[, c('customers_out_ten_min_by_county',
                  'customers_served_ten_min_by_county') := list(sum(customers_out_counts),
                                                                sum(customers_by_pod)),
              by = c("counties", "datetimes")]
  ...
  sim_dat_b <- unique(
    ...
    by = c(
      "counties",
      "datetimes",
      "customers_out_ten_min_by_county",
      "customers_served_ten_min_by_county"
    )
  )
  ...
  sim_dat_b <-
    sim_dat_b[, c('customers_out_hourly', 'customers_served_hourly') :=
                list(round(mean(customers_out_ten_min_by_county), digits = 0),
                     round(mean(customers_served_ten_min_by_county), digits = 0)),
              by = c("counties", 'hour')]
  ...
  sim_dat_b <-
    sim_dat_b[, .(counties, hour, customers_out_hourly, customers_served_hourly)]
  
  simed_counties <- sim_dat_b %>% group_by(counties) %>% group_split()
  
  simed_counties <- as.list(simed_counties)
  ...
  num_outages_frame <- num_outages_frame %>% filter(county_num_id != 'testcase')
  ...
  saveRDS(
    ...
    here(
      ...
      "cleaned_12_hrs",
      paste0("days_exposed_unexposed_12_hrs_", p, ".RDS")
    )
  )
  saveRDS(
    num_outages_frame,
    here(
      ...
      "power_outage_simulation_cleaned_data",
      "cleaned_12_hrs",
      paste0("num_outages_g_12_hrs_", p, ".RDS")
    )
  )
